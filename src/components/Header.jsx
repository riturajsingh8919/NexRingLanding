...
const Header = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 py-4 transition-transform duration-300 ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}>
      <div className="container mx-auto px-4 lg:px-16">
        ...
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-full px-6 py-3">
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center gap-8">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className="text-white/70 text-lg font-normal hover:text-white transition-colors duration-300"
                >
                  {link.name}
                </Link>
              ))}
            ...
            {/* Right Side - White Glassmorphism Button */}
      ...
      {/* Mobile Menu */}
      <div className={`md:hidden absolute top-full left-0 right-0 px-4 lg:px-16 mt-2 transition-all duration-300 ${isMobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
